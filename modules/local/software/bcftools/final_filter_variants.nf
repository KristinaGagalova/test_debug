process FINAL_FILTER_VARIANTS {
    ...
    input:
    tuple val(meta), path(snps), path(snps_idx), path(indels), path(indels_idx)

    output:
    tuple val(meta), path("${meta.id}.filtered.biallelic.*.snps.vcf"), emit: vcf
    ...
    def output_filename = "${prefix}.filtered.biallelic.${alleles_suffix}.${missing_suffix}.1random${window_suffix}.${ld_suffix}.snps.vcf"
    """
    bcftools view \\
        ...
        ${args1} \\
        ${snps} \\
        -o ${prefix}.filtered.biallelic.snps.bcf
	
    bcftools +prune \\
        -m ${ld_threshold} \\
        -w ${window_size} \\
        ${prefix}.filtered.biallelic.snps.bcf \\
        -O${output_format} \\
        -o ${output_filename} \\
        ${args2}
